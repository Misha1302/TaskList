...
app.MapPost("/login", (Person p) =>
{
    // находим пользователя 
    var person = people.FirstOrDefault(x => x.Email == p.Email && x.Password == p.Password);
...
app.MapPost("/authorize", (Person p) =>
{
    // находим пользователя 
    var person = people.FirstOrDefault(x => p.Email == x.Email && p.Password == x.Password);
    // если пользователь не найден, отправляем статусный код 401
